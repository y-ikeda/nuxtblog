<template>
  <aside class="is-medium menu">
    <ul class="menu-list">
      <li class="is-right">
        <a @click="get_newpost()" class="is-active"
          ><i class="fab fa-css3-alt"></i>新着情報</a
        >
      </li>
    ...
    <ul class="menu-list">
      <li v-for="(post, index) in posts" :key="index">
        <a @click="singlelink(post.slug)" class="tag is-white is-medium"
          >{{ post.title }}({{ get_date(post) }})</a
        >
      </li>
    </ul>
  </aside>
...
export default {
  methods: {
    IsActive:function(key){
      if(key == "new" && this.$store.state.global.mode == "newlist")return "is-active"

      return ""
    },
    get_newpost:function(){
      this.$store.commit("category/setpath", "");
      this.$store.commit("global/setkey", "");
